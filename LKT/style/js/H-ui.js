...
$("#ajaxSe").change(function(){
    let pagesize=$(this).val();
    $(".pagesize_class").val(pagesize);
    var page = 1;
    location.replace(location.href+'&page='+page+'&pagesize='+pagesize);
...
window.onload=function(){
    $("[type=reset]").text("重  置");
    $("[type=reset]").css("backgroundColor","#fff")
    $("[type=reset]").val("重  置");
	if(!$('.paginationDiv').parent().hasClass('tab_footer')){
		$('.paginationDiv').parent().addClass('tab_footer')
	}
}