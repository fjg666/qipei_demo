...
$.alerts = function (args) {
    ...
    var id = $.randomString();
    var html = '';
    html += '<div class="up_body" data-backdrop="static" id="' + id + '">';
    html +='<div class="up_flex">',
    html += '<div class="up_auto" role="document">';
    html += '<div class="up_content">';
    html += '<div class="up_title rounded-0">';
    html += '<img src="images/icon/ganh.png" class="up_title_l" />'+ args.title+'<img src="images/icon/cha.png"/ class="up_title_r" data-dismiss="modal">';
    html +='</div>';
    html +='<div class="up_word">' + content + '</div>';
   	html += '<div class="up_footer">';
    html += '<button class="up_btn btn btn-primary alert-confirm-btn panel_btns" data-dismiss="modal">' + confirmText + '</button>';
    html += '</div>';
    html += '</div>';
    html += '</div>';
    html += '</div>';
...
$(document).on('click', '#file_select_modal .file-more', function () {
	...
	if($(".upload_list .upload_mask").attr("data-id")>0){
		group_id=$(".upload_list .upload_mask").attr("data-id")
	}
...
$(document).ready(function () {
    ...
    $(document).on('click', '.auto-form .auto-form-btn2', function () {
        var form = $(this).parents('.auto-form');
        var text_name = $(this).parents('.sewo').children(".breadcrumb").children("a").text();
        console.log(form,text_name)
        submits(form,text_name);
        return false;
    });
    ...
    function submits(form,text_name) {
        var btn = $(form.find('.auto-form-btn2'));
...
$(document).on('click', '.upload-group .select-file', function () {
    ...
    if(max){
        var ppw = group.find('.upload-preview').length;
        if(Number(ppw) >= Number(max)){
            layer.msg('上传图片数量超出限制！', {
                time: 2000
            });
            return false;
        }
        console.log(btn.attr('data-max'));
    }
        ...
        success: function (res) {
            if (group.hasClass('multiple')) {
                ...
                if (preview.find('.file-item-input').val() == '') {
                    ...
                    preview_item.innerHTML = preview.prop('outerHTML');
                    preview_item = $(preview_item).find('.upload-preview');
                    if(preview_item.find('.form_new_words')){
                        preview_item.find('.form_new_words').removeClass('set_center');
                        preview_item.find('.form_new_words').text('设为主图');
                        preview_item.find('.form_new_words').next().attr("name","imgurls[]"); 
                    }
                    var file_item_input = preview_item.find('.file-item-input');
                    var file_item_preview_img = preview_item.find('.upload-preview-img');
...
$(document).on('click', '.upload-group .delete-file', function () {
    ...
    preview_img.attr('src', './images/class_noimg.jpg');
});

$(document).on('click', '.upload-group .file-item-delete-c', function () {
    var btn = $(this);
    var group = btn.parents('.upload-group');
    var input = group.find('.file-input');
    var preview_img = group.find('.upload-preview-img');
    input.val('').trigger('change');
    preview_img.attr('src', './images/icon1/add_g_t.png');
});
...
$(document).on('click', '.upload-group .file-item-delete-pp', function () {
    var btn = $(this);
    var preview = btn.parents('.upload-preview');
    if (preview.siblings('.upload-preview').length == 0) {
        preview.find('.file-item-input').val('');
