...
$reqURL = handle_url($url9,false);//true 不需要  false 需要
...
function save_image($imageData){
    ...
    if(strpos($imageData, "\",\"")){
        ...
        foreach ($var as $value){
            $i++;
            $imgName = "E:/lkt/admin/express/sf_".$showtime."-".$i.".jpg";
            generate_image($imageData, $imgName);
        }
    }else{
        $imgName = "E:/lkt/admin/express/sf_".$showtime.".jpg";
        generate_image($imageData, $imgName);
    }
}
...
{    
    ...
    function assembly_param($post,$fengmi){
        ...
        $waybillDto = new WaybillDto();
        //这个必填
        $waybillDto->appId = "AX"; //对应丰桥平台获取的clientCode
        $waybillDto->appKey = "wBAIrVpEbb3CkpIuke5rlFrzkD6hnLbB"; //对应丰桥平台获取的checkWord
        $waybillDto->monthAccount = "7551234567"; //月结卡号
        ...
        foreach ($post['goods'] as $k => $v) {
            ...
            if ($k > 0) {
                $cargoInfoList = array($cargoInfoList,$cargo[$k]);
            }else{
        ...
        if ($fengmi)
        {
            ...
            $rlsMain->printIcon = "1";
            // $rlsMain->proCode = "T4";
            $rlsMain->qrcode = "MMM={'k1':'(目的地中转场代码)','k2':'".$post['destcode']."','k3':'(目的地单元区域)','k4':'(附件通过三维码(express_type_code、 limit_type_code、 cargo_type_code)映射时效类型)','k5':'".$post['remark']."','k6':'(AB标识)','k7':'(校验码)'}";
            // $rlsMain->sourceTransferCode = "021WTF";
            // $rlsMain->waybillNo = "SF7551234567890";
            // $rlsMain->xbFlag = "XB";
