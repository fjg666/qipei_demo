...
class EnterAction extends Action {
    ...
    public function getDefaultView() {
        ...
        $condition = "a.store_id = '$store_id' and a.recycle = 0 and b.type = 0";
        ...
        $sql0 = "select a.product_number,a.product_title,a.imgurl,a.status,a.mch_id,c.id,c.pid,c.price,c.attribute,b.total_num,b.flowing_num,b.add_date from lkt_stock as b left join lkt_product_list as a on b.product_id = a.id left join lkt_configure as c on b.attribute_id = c.id left join lkt_mch as d on a.mch_id = d.id where $condition order by b.add_date desc";
        $r0 = $db->select($sql0);
        $total = count($r0);

        $sql1 = "select a.product_number,a.product_title,a.imgurl,a.status,a.mch_id,c.id,c.pid,c.price,c.attribute,b.total_num,b.flowing_num,b.add_date from lkt_stock as b left join lkt_product_list as a on b.product_id = a.id left join lkt_configure as c on b.attribute_id = c.id left join lkt_mch as d on a.mch_id = d.id where $condition order by b.add_date desc limit $start,$pagesize";
        $r1 = $db->select($sql1);
        if($r1){
            foreach ($r1 as $k => $v){
                $mch_id = $v->mch_id;
        ...
        $url = "index.php?module=stock&action=Enter&pagesize=".urlencode($pagesize)."&product_number=".urlencode($product_number)."&product_title=".urlencode($product_title)."&startdate=".urlencode($startdate)."&enddate=".urlencode($enddate);
        ...
        if($excel_sql != ''){
            $sql2 = "select a.product_number,a.product_title,a.imgurl,a.status,a.mch_id,c.id,c.pid,c.price,c.attribute,b.total_num,b.flowing_num,b.add_date from lkt_stock as b left join lkt_product_list as a on b.product_id = a.id left join lkt_configure as c on b.attribute_id = c.id left join lkt_mch as d on a.mch_id = d.id $excel_sql";
        }else{
            $sql2 = "select a.product_number,a.product_title,a.imgurl,a.status,a.mch_id,c.id,c.pid,c.price,c.attribute,b.total_num,b.flowing_num,b.add_date from lkt_stock as b left join lkt_product_list as a on b.product_id = a.id left join lkt_configure as c on b.attribute_id = c.id left join lkt_mch as d on a.mch_id = d.id $condition";
        }
        $r2 = $db->select($sql2);
        if($r2){
            foreach ($r2 as $k => $v){
