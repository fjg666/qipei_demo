...
class ListAction extends Action {
    ...
    public function getDefaultView() {
        ...
        $condition = " a.store_id = '$store_id' and c.store_id = '$store_id' and a.status != 0 ";
        $condition1 = " a.store_id = '$store_id' and c.store_id = '$store_id' and a.status != 0 ";
        ...
        $list = array();
        $sql = "select a.id,a.user_id,a.name,a.add_date,a.money,a.s_charge,a.mobile,a.status,a.refuse,b.Cardholder,b.Bank_name,b.Bank_card_number,c.source from lkt_withdraw as a left join lkt_user_bank_card as b on a.Bank_id = b.id left join lkt_user as c on a.user_id = c.user_id where $condition ";
        $r = $db->select($sql);
        $total = 0;
        if($r){
            $total = count($r);
            ...
            if($pageto == 'whole') { // 导出全部
                ...
                $db->admin_record($store_id,$admin_name,' 导出全部提现记录列表 ',4);
                $sql = "select a.id,a.user_id,a.name,a.add_date,a.money,a.s_charge,a.mobile,a.status,a.refuse,b.Cardholder,b.Bank_name,b.Bank_card_number,c.source from lkt_withdraw as a left join lkt_user_bank_card as b on a.Bank_id = b.id left join lkt_user as c on a.user_id = c.user_id where $condition1 order by a.add_date desc ";

            }else if($pageto == 'inquiry'){//导出查询

                $db->admin_record($store_id,$admin_name,' 导出提现记录列表全部查询数据！ ',4);
                $sql = "select a.id,a.user_id,a.name,a.add_date,a.money,a.s_charge,a.mobile,a.status,a.refuse,b.Cardholder,b.Bank_name,b.Bank_card_number,c.source from lkt_withdraw as a left join lkt_user_bank_card as b on a.Bank_id = b.id left join lkt_user as c on a.user_id = c.user_id where $condition order by a.add_date desc";
            }else if($pageto == 'This_page'){//导出当前页

                $db->admin_record($store_id,$admin_name,' 导出当前页提现记录 ',4);
        ...
        $url = "index.php?module=finance&action=List&name=".urlencode($name)."&mobile=".urlencode($mobile)."&startdate=".urlencode($startdate)."&enddate=".urlencode($enddate)."&pagesize=".urlencode($pagesize);
