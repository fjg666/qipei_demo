...
class IndexAction extends Action {
    ...
    public function getDefaultView() {
        ...
        if($r){
            foreach ($r as $k => $v){
                $r_title = $v->title;
                $res = substr(strtoupper($this->encode($r_title)),0,2) . '_';
                $id = $v->id;
                $v->id_id = $res . $v->id;
                $v->s_id = $res . $v->s_id;
                $rew[$num++] = $v;
            ...
            $sql = "select id,s_id,title,type,add_time from lkt_core_menu where $condition and recycle = 0 and level < 3 order by id";
            $r1 = $db->select($sql);
            if($r1){
                $num = 0;
                foreach ($r1 as $k => $v){
                    ...
                    // 根据上级id,查询上级信息
                    $sql = "select title from lkt_core_menu where recycle = 0 and id = '$s_id'";
                    $r2 = $db->select($sql);
                    $r_title = $r2[0]->title;
                    $res = substr(strtoupper($this->encode($r_title)),0,2) . '_';
                    $id = $v->id;
                    $v->id_id = $res . $v->id; // id
                    $v->s_id = $res . $v->s_id; // 上级id
                    $v->title = $v->title . $type1; // 类型
                    $rew[$num++] = $v;
                    // 根据id，查询下级信息
                    $sql = "select id,s_id,title,add_time from lkt_core_menu where s_id = '$id' and recycle = 0 order by id";
                    $rr = $db->select($sql);
                    if($rr){
                        foreach ($rr as $ke => $va){
                            $id1 = $va->id;
                            ...
                            $va->s_id = $res . $va->s_id;
                            $rew[$num++] = $va;
                            $sql = "select id,s_id,title,add_time from lkt_core_menu where s_id = '$id1' and recycle = 0 order by id";
                            $rrr = $db->select($sql);
                            if($rrr){
                                foreach ($rrr as $key => $val){
                                    $id2 = $val->id;
                                    ...
                                    $val->s_id = $res . $val->s_id;
                                    $rew[$num++] = $val;
                                    $sql = "select id,s_id,title,add_time from lkt_core_menu where s_id = '$id2' and recycle = 0 order by id";
                                    $rrrr = $db->select($sql);
                                    if($rrrr){
                                        foreach ($rrrr as $key1 => $val1){
                                            $val1->id_id = $res . $val1->id;
        ...
        $url = "index.php?module=menu&action=Index&title=".urlencode($title)."&pagesize=".urlencode($pagesize);
