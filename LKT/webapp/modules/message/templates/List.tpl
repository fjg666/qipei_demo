...
<div class="pd-20 page_absolute">
    ...
    <div class="swivch page_bgcolor swivch_bot">
        ...
        {if $button[0] == 1}
            <a href="index.php?module=message&action=List" class="btn1 active swivch_active" style="border-right: 1px solid #ddd!important;">短信模板</a>
        {/if}
        {if $button[1] == 1}
    ...
    {if $store_type == 0}
        <div style="clear:both;" class="page_bgcolor">
            <a class="btn newBtn radius" href="index.php?module=message&action=Addlist"><img src="images/icon1/add.png"/>&nbsp;添加模板</a>
        </div>
    {/if}
    <div class="page_h16"></div>
        ...
        <table class="table-border tab_content">
            ...
            {foreach from=$list item=item name=f1}
                <tr class="text-c tab_td">
                    ...
                    <td style="text-align: left;">{$item->TemplateCode}</td>
                    <td>{$item->content}</td>
                    <td class="tab_time">{$item->add_time}</td>
                    <td class="tab_editor">
                        {if $button[3] == 1}
                            <a style="text-decoration:none" class="ml-5" href="index.php?module=message&action=Modifylist&id={$item->id}" title="编辑" >
                                <img src="images/icon1/xg.png"/>&nbsp;编辑
                            </a>
                        {/if}
                        {if $button[4] == 1}
                            <a title="删除" href="javascript:;" onclick="confirm('确定要删除此管理员吗?',{$item->id},'index.php?module=message&action=Dellist&id=')" class="ml-5" style="text-decoration:none">
