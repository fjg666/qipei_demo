...
class ProvinceAction extends Action {
	public function getDefaultView() {
        ...
        $sel_city = $request->getParameter('sel_city');
        $sel_city_str = "";
        $wherestr = '';

        if($sel_city != array() && $sel_city[0] != ''){
            foreach ($sel_city as $k => $v){
                $sel_city_str .=$v;
            }
            $sel_city_str = substr($sel_city_str,0,-1);
            $wherestr = "and GroupID not in ($sel_city_str)";
        }
        $sql = "select GroupID,G_CName from admin_cg_group where G_ParentID = 0 $wherestr";
        $r = $db->select($sql);
