...
class SendAction extends Action {
    ...
    public function getDefaultView() {
      ...
      $store_id = $this->getContext()->getStorage()->read('store_id');
  		$sql = "select id from lkt_admin where id ='$admin_id'";
  		$res = $db->select($sql);
  		...
  		foreach ($recip as $key => $value) {
  			...
  			$sql = "insert into lkt_system_message(store_id,senderid,recipientid,title,content,time) " .
              "values('$store_id','$senderid','$value','$title','$content','$datatime')";
      	$r = $db->insert($sql);
      	$conr += $r;
