    ...
    <div class="text-c">
        <form method="get" action="index.php" name="form1" class='iframe-search'>
            <div style="border-left:solid 1px #fff;">
                ...
                <input type="text" class="input-text"  autocomplete="off" style="width:200px" placeholder="请输入会员id" name="user_id" value="{$user_id}">
                <input type="text" class="input-text"  autocomplete="off" style="width:200px" placeholder="请输入会员名称" name="user_name" value="{$user_name}">
                ...
                <select name="is_out" class="select" style="width: 140px;vertical-align: middle;background-color: rgb(255, 255, 255);">
                    ...
                    <option value="0" {if $is_out === '0'}selected="selected"{/if}>未过期</option>
                    <option value="1" {if $is_out === '1'}selected="selected"{/if}>已过期</option>
                </select>
                ...
                <input type="text" class="input-text" autocomplete="off" style="width:200px" placeholder="请输入手机号码" name="tel" value="{$tel}">
        ...
        <table class="table-border tab_content">
            ...
            {foreach from=$list item=item name=f1}
                <tr class="text-c tab_td">
                    ...
                    <td class="tab_imgurl"><image class='pimg' src="{$item->headimgurl}" /></td>
                    <td class="tab_title" style="text-align: center!important;">{$item->user_name}</td>    
                    <td class="tab_title" style="text-align: center!important;">{$item->zhanghao}</td>
                    <td class="tab_title" style="text-align: center!important">{$item->grade}</td>
                    <td class="tab_title" style="text-align: center!important;">{$item->mobile}</td>
                    <td class="tab_title" style="text-align: center!important;">￥{$item->money}</td>
                    <td class="tab_title" style="text-align: center!important;">{$item->score}</td>
                    ...
                    <td class="tab_num">{$item->z_num}</td>
                    <td class="tab_num">￥{$item->z_price}</td>
                    <td class="tab_num">{$item->share_num}</td>
                    <td class="tab_num">
                        {if $item->is_out == 1}
                            是
                        {else}
                    ...
                    <td class="tab_num">{$item->Register_data}</td>
                    <td   class="tab_five">
                        {if $button[1] == 1}
                            <a  href="index.php?module=userlist&action=View&id={$item->user_id}" title="查看" >
                                <img src="images/icon1/ck.png"/>&nbsp;查看
                            </a>
                        {/if}
                        {if $button[3] == 1}
                            <a  title="编辑" href="index.php?module=userlist&action=Modify&id={$item->user_id}">
                        ...
                        {if $button[2] == 1}
                            {if $item->level == ''}
                            <a  href="javascript:void(0);" onclick="confirm('确定要删除此用户吗?','{$item->user_id}','index.php?module=userlist&action=Del&id=','删除')">
							...
							<div class='gd_div'>
								<a href="javascript:;" title="更多" style='width: 56px!important;display: flex;align-items: center;justify-content: center;margin: 0;'>
								 ...
								 <ul style='position: absolute;top: 22px;background: #FFFFFF;z-index: 99;left: 0;display: none;'>
								 	<li onclick="refuse(this,'{$item->user_id}','money','请输入充值的余额，扣除添加负号')"><span>余额充值</span></li>
								 	<li onclick="refuse(this,'{$item->user_id}','score','请输入充值的积分金额，扣除添加负号')"><span>积分充值</span></li>
								 	<li onclick="lv_modify('{$item->user_id}')"><span>等级修改</span></li>
								 </ul>
							</div>
							 <!-- <a href="javascript:;" title="充值余额" onclick="refuse(this,'{$item->user_id}','money','请输入充值的余额，扣除添加负号')">
								  <img src="images/icon1/yecz.png"/>&nbsp;充值余额
							  </a>
							  <a href="javascript:;" title="充值积分" onclick="refuse(this,'{$item->user_id}','score','请输入充值的积分金额，扣除添加负号')">
