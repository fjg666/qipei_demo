...
<div class="pd-20 page_absolute">
    <div class="text-c text_c">
        <form name="form1" id="form1" action="index.php" method="get">
            ...
            <select name="active" class="select input_180"  id="active" onClick="activitytype();">
                <option value="0" {if $active == 0}selected="selected"{/if}>请选择商品类型</option>
                <option value="1" {if $active == 1}selected="selected"{/if}>正价</option>
                {$select4}
                <option value="6" {if $active == 6}selected="selected"{/if}>会员</option>
            </select>
            ...
            <select name="show_adr" class="select input_180" id="show_adr" {if $active != 1}style="display: none;"{/if}>
                {$select3}
            </select>
            <input type="text" name="product_title" size='8' id="product_title" value="{$product_title}" placeholder="请输入商品名称" class="input-text input_180">
            <input type="text" name="mch_name" id="mch_name" value="{$mch_name}" placeholder="请输入店铺名称" class="input-text input_180">
    ...
    <div style="clear:both;margin-top:0!important;background-color: #edf1f5;" class="btnDiv">
        {if $button[0] == 1}
            {if $mch_id == '0'}
                <a class="btn btn1 radius" id="btn1" style="background-color:#38b4ed;color: #fff;height: 36px;" onclick="tc({$item->id})" href="javascript:void(0);">
        ...
        <table class="table-border tab_content">
            ...
            {foreach from=$list item=item name=f1}
                <tr class="text-c tab_td">
                    <td class="tab_label">
                        <div class="tab_auto">
                            <input name="id[]" id="{$item->id}" type="checkbox" class="inputC product_select" value="{$item->id}" onclick="onshelves({$item->id})">
                    ...
                    <td class="tab_title ">{$item->product_title}
                        <div class="tab_clear">
                    ...
                    <td>{$item->volume}</td>
                    <td class="tab_time">{$item->upper_shelf_time}</td>
                    <td class="tab_three">
                       <div>
						   {if $button[1] == 1}
						       {if $item->status == 2}
						           <a onclick="modify(this,{$item->status},'index.php?module=product&action=Modify&id={$item->id}')" style="color: rgb(136, 143, 158);">
						               <img src="images/icon1/xg.png"/>&nbsp;编辑
						           </a>
						       {else}
						           <a href="index.php?module=product&action=Modify&id={$item->id}" style="color: rgb(136, 143, 158);" title="编辑">
						               <img src="images/icon1/xg.png"/>&nbsp;编辑
						           </a>
						       {/if}
						   {/if}
						   {if $button[2] == 1}
						       <a onclick="del({$item->id},'index.php?module=product&action=Del&id=')" style="color: rgb(136, 143, 158);">
						           <img src="images/icon1/del.png"/>&nbsp;删除
						       </a>
						   {/if}
                           {if $button[6] == 1}
                               <a href="index.php?module=product&action=Copy&id={$item->id}" title="复制" style="">
                                   <img src="images/icon1/copy.png"/>&nbsp;复制
                               </a>
                           {/if}
                           {if $button[7] == 1}
                               {if $item->upper_status}
                                   <a onclick="move_upward('{$item->id}','{$item->upper_id}','{$item->underneath_id}','{$item->upper_status}')" href="javascript:void(0);" title="上移">
                                       <img src="images/icon1/moveUp.png" style='width: 12px; height: 12px;'/>&nbsp;上移
                                   </a>
                               {else}
                                   <a onclick="move_upward('{$item->id}','{$item->upper_id}','{$item->underneath_id}','{$item->upper_status}')" href="javascript:void(0);" title="下移">
                                       <img src="images/icon1/moveDown.png" style='width: 12px; height: 12px;'/>&nbsp;下移
                                   </a>
                               {/if}
                               <a onclick="on_top({$item->id})" href="javascript:void(0);" title="置顶">
						   ...
						   {if $button[5] == 1}
						       {if $item->status == 2}
						           <a onclick="aj({$item->id})" href="javascript:void(0);" title="下架">
						               <img src="images/icon1/xj.png"/>&nbsp;下架
						           </a>
						       {else}
						           <a onclick="aj({$item->id})" href="javascript:void(0);" title="上架">
    ...
    <div class="tab_footer">{$pages_show}</div>
</div>
            ...
            <form action="javascript:void(0);" name="form2" id='form2' method="post">
                ...
                <div class="upload-group">
                    <div class="input-group row" style="margin-left: 0px !important;">
                        ...
                        <div>
                            <input index="0" value="{$logo}" name="logo" class="form-control file-input"
