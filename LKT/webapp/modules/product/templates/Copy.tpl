...
<nav class="nav-title">
	...
	<span><span class="arrows">&gt;</span>复制</span>
</nav>
		...
		<div class="formDivSD">
			...
			<div class="formContentSD">
				...
				<div class="formListSD">
					...
					<div class="formInputSD"><input type="text" value="{$subtitle}" name="subtitle">
					...
					<div class="formInputSD" style='display: block;'>
						<div class='selectDiv' onclick="select_class()">
							<select name="product_class" class="select" readonly="readonly"  style='margin-right: 0;'>
								<option selected="selected" value="{$class_id}"></option>
							</select>
							<div id="div_text">
								{foreach from=$ctypes item=item name=f1 key=k}
									{if $smarty.foreach.f1.first}
										<p class='selectItem' id='p{$k+1}' tyid='{$item->cid}' onclick='del_sel(this,{$item->level},{$item->cid})'>{$item->pname}</p>
									{else}
										<p class='selectItem' id='p{$k+1}' tyid='{$item->cid}' onclick='del_sel(this,{$item->level},{$item->cid})'><span>&gt;</span>{$item->pname}</p>
									{/if}
								{/foreach}
							</div>
						</div>
				...
				<div class="formListSD">
					...
					<div class="formInputSD">
						<select name="brand_class" class="select" id="brand_class">
							{foreach from=$brand_class item=item name=f1}
								<option value="{$item->brand_id}" {if $item->brand_id == $brand_id}selected="selected"{/if}>{$item->brand_name}</option>
							{/foreach}
						</select>
					</div>
				</div>
		...
		<div class="formDivSD">
			...
			<div class="formContentSD">
				<div class="form_li form_num formListSD">
					...
					<div class="form_new_r"><span class="form_li_one">当前库存量小于</span><input type="number" min="0" oninput="value=value.replace(/[^\d]/g,'')" required="required" value="{$min_inventory}" name="min_inventory"><span class="form_li_one">时，商品库存报警</span><span>（可在库存列表中查看明细，预警数据将以红色加粗显示）</span></div>
				</div>
				...
				<div class="form_li formListSD">
					...
					<div class="form_new_r"><input type="number" required="required" min="0" oninput="value=value.replace(/[^\d]/g,'')" value="{$sort}" name="sort" style="padding-left: 10px;"></div>
				</div>
				<div class="form_li formListSD">
				...
				<div class="form_li">
					...
					<div class="form_new_r">
						<div class="ra1">
							<input name="active" onchange="active_select(this)" type="radio" {if $active == 1}checked="checked"{/if} style="display: none;" id="active-1" class="inputC1" value="1">
							<label for="active-1">正价</label>
						</div>
						{$Plugin_arr.res1}
						<div class="ra1">
							<input name="active" onchange="active_select(this)" type="radio" {if $active == 6}checked="checked"{/if} style="display: none;" id="active-6" class="inputC1" value="6">
				...
				<div class="form_li">
					...
					<div class="form_new_r form_yes">
						<div class="ra1">
							<input name="is_hexiao" onchange="form_address(this)" type="radio" {if $is_hexiao == 1}checked="checked"{/if} style="display: none;" id="is_hexiao-1" class="inputC1" value="1">
							<label for="is_hexiao-1">是</label>
						</div>
						<div class="ra1">
							<input name="is_hexiao" onchange="form_address(this)" type="radio" {if $is_hexiao == 0}checked="checked"{/if} style="display: none;" id="is_hexiao-2" class="inputC1" value="0">
				...
				<div class="form_address" id="xd_hx" {if $is_hexiao == 0}style="display: none;"{/if}>
					<span>核销地址：</span>
					<input type="text" name="hxaddress" value="{$hxaddress}">
				</div>
			</div>
			<div class="formSpaceSD"></div>
		</div>
...
<script>
    ...
    var page = new Vue({
        ...
        data: {
            sub_cat_list: [],
            attrTitle: JSON.parse('{/literal}{$attr_group_list}{literal}', true), //可选规格数据
            strArr: JSON.parse('{/literal}{$checked_attr_list}{literal}', true), //已选规格数据
		...
		created(){
			var list=JSON.parse('{/literal}{$attr_group_list}{literal}', true)
	...
	function check() {
		console.log(66);
		var url = 'index.php?module=product&action=Copy';
		if(!t_check){
			layer.msg('请勿重复提交！', {
				time: 2000
			});
