...
<div class="iframe-content">
    ...
    <div class="text-c tetx_c">
        <form name="form1" action="index.php" method="get" class="page_list">
            ...
            <input type="text" name="p_name" size='8' value="{$p_name}" id="p_name_" placeholder="请输入订单号" style="width:200px"
                   class="input-text">
        ...
        <table class="table table-border table-bordered table-bg table-hover table-sort taber_border tab_content">
            ...
            {foreach from=$list item=item name=f1}
                <tr class="text-c tab_td">
                    ...
                    <td>{$item->r_sNo}</td>
                    <td>
                        {if $item->real_money == '0.00' && $item->re_type != 3}
                            未退款
                    ...
                    <td class="tab_nowrap">
                        {if $item->r_status == 4}
                            ...
                            {elseif $item->r_type == 4 || $item->r_type == 9}
                                <span>审核通过</span>
                                {*<span>已退款</span>*}
                            {elseif $item->r_type == 4 }
                                <span>售后完成</span>
                            {elseif  $item->r_type == 11}
                                <span>退换中</span>
                            {elseif  $item->r_type == 12}
                                <span style="color: #7CCD7C;">退换完成</span>
                            {else}
                                <span>已拒绝{$item->r_type}</span>
                    ...
                    <td class="tab_three">
                        ...
                        {if $button[1] == 1}
                            {if $item->r_status == 4}
                                {if $item->r_type == 0}
                                    {if $item->re_type == 1}
                                        {if $item->pay == 'baidu_pay'}
                                            <a href="javascript:;" title="审核通过"
                                               onclick="is_ok(this,{$item->id},1,'index.php?module=return&action=Examine','确认要退款到用户钱包并让用户寄回吗？')">
                                    ...
                                    {elseif $item->re_type == 2}
                                        {if $item->pay == 'baidu_pay'}
                                            <a href="javascript:;" title="审核通过"
                                               onclick="is_ok(this,{$item->id},9,'index.php?module=return&action=Examine','确认要退款到用户钱包吗？')">
                                                <img src="images/icon1/qy.png"/>&nbsp;通过
                                            </a>
                                        {else}
                                            <a href="javascript:;" title="审核通过"
                                    ...
                                    {/if}
                                {elseif $item->r_type == 3 && $item->r_type != 11}
                                    ...
                                    {else}
                                        <a href="javascript:;" title="回寄"
                                           onclick="zxfahuo('{$item->r_sNo}','{$item->id}','{$item->re_type}')">
                                ...
                                {/if}
                            {elseif $item->r_type != 8 &&($item->re_type == 3 && $item->r_type == 11)}
                                <a href="javascript:;" title="回寄"
                                   onclick="zxfahuo('{$item->r_sNo}','{$item->id}','{$item->re_type}')">
                                    <img src="images/icon1/sx.png"/>&nbsp;回寄
                                </a>
            ...
            <div class="row cl">
                <div class="col-9 " style="margin-left:40%">
                    <input class="qd closeMask" type="button"
                           onclick="qd('index.php?module=orderslist&action=Addsign',3)" value="{$item->r_type}提交">
                </div>
            </div>
        </form>
    </div>
