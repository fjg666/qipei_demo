 ...
 * @Author: your name
 * @Date: 2019-09-16 09:12:00
 * @LastEditTime: 2019-09-30 16:00:52
...
<div class="" style="margin: 0px 10px;" >
    ...
    <div class="swivch swivch_bot page_bgcolor" style="font-size: 16px;">
        <a href="index.php?module=invoice" class="btn1" style="height: 42px !important;border: none!important;border-top-left-radius: 2px;border-bottom-left-radius: 2px;">发货单</a>
        <a href="index.php?module=invoice&action=indexx" class="btn1 swivch_active" style="height: 42px !important;border: none!important;border-top-left-radius: 2px;border-bottom-left-radius: 2px;">快递单</a>
    ...
    <div class="text-c" style="display: flex;">
        <form name="form1" action="index.php" method="get" style="width: 100%;padding: 10px;">
            ...
            <input type="hidden" name="action" value="indexx" />
            ...
            <select name="r_time" class="select" id="r_time" style="width: 100px;height: 31px;vertical-align: middle;">
                ...
                <option value="4" {if $r_time==4}selected{/if}>打印时间</option>
            </select>
            ...
            <input type="text" class="input-text" style="width:150px" placeholder="请输入运单号" name="expresssn" id="expresssn" value="{$expresssn}">
        ...
        <table class="table-border tab_content">
            ...
            {foreach from=$list item=item name=f1}
               <tr class="text-c tab_td">
                    ...
                    <td>{$item->express}<br />{$item->expresssn}</td>
                    <td>{$item->num}</td>
                    <td>{$item->weight}</td>
    ...
    <div style="position: fixed;bottom: 0;background-color: #fff;width: 98%;">
        ...
        <div style="margin-left: 60px;float: left;padding-top: 24px;">选择快递模版：
            <select name="print_type" class="select" id="print_type" style="width: 150px;height: 31px;vertical-align: middle;">
                {foreach from=$tpl item=item name=f1}
                <option value="{$item->id}" {if $item->id==1}selected{/if}>{$item->name}</option>
                {/foreach}
            </select>
        </div> -->
        <div style="margin-left: 60px;float: left;padding-top: 24px;"><span style="color:rgba(65,70,88,1);">打印份数设置：</span><input style="width: 60px;" type="number" id="putnum" value="1">份</div>
        <div style="float:  right;margin-right: 60px;width: 112px;">
            <button onclick="print()" type="button" class="btn btn-success" style="margin-top: 25px;">生成快递单</button>
        </div>
        ...
        <div class="modal-dialog" style="z-index: 10000;max-width: 840px;box-shadow:0 0px 0px 0px rgba(0, 0, 0, 0);display:flex;">
            <div class="modal-content radius box-modal" style="width: 840px;">
                ...
                <form name="form1" id="form1" class="form form-horizontal" method="post" enctype="multipart/form-data" >
                            ...
                            <div style="margin-left: 135px;height: 150px;overflow: auto;" >
                                <div class="col-xs-4 col-sm-10" style="display: flex;" id="items" v-for="(item,index) in data.goods" :key="index" >
                                    ...
                                    <span style="margin-left: 8PX;">
                                        <input type="text" readonly="readonly" placeholder="商品编号" v-model="item.product_number" size="10"/>
                                    </span>
                                    <span>
                                        <input type="text" readonly="readonly" placeholder="规格/属性" v-model="item.size" size="10"/>
                                    </span>
                                    <span>
                                        <input type="text" readonly="readonly" placeholder="单价" v-model="item.p_price" size="10"/>
                                    </span>
                                    <span>
                                        <input type="text" readonly="readonly" placeholder="数量" v-model="item.num" size="10"/>
                                    </span>
                        ...
                        <div class="row" style="margin-left: 3px;">
                            <label class=" col-xs-4 col-sm-2">快递信息：</label>
                            <div class="col-xs-4 col-sm-4">
                                <input type="text" v-model="data.express" id="express" readonly=""></input>
                                <input type="text" v-model="data.expresssn" id="expresssn" readonly=""></input>
                            </div>
                        </div>
...
<script type="text/javascript">
    ...
    function print(){
        ...
        // var url = "index.php?module=invoice&action=indexx&m=put_print&id="+id;
        var url = "index.php?module=invoice&action=indexx&m=go_print&id="+id;
            ...
            success: function (res) {
                ...
                if (res.code == 200) {
                    ...
                    for (var i = 0; i < res.data.length; i++) {
                        text += `<img src="${res.data[i]}" style="width: 50%;"><div style="page-break-after: always;"></div>`;
                    }
    ...
    var app = new Vue({
        ...
        methods:{
            ...
            modelCheck:function(){
                ...
                $.ajax({
                    type: "post",
                    url: "index.php?module=invoice&action=indexx&m=edit",
                    async: true,
                    data: this.data,
                    success: function (res) {
                        console.log(res);
