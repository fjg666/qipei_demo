...
class IndexAction extends Action
{
    ...
    public function getDefaultView()
    {
        ...
        // 查询推广图，根据sort顺序排列
        $sql = "select * from lkt_extension where store_id = '$store_id' and store_type = '$store_type' order by sort";
        $r_pager = $db->select($sql);
        $total = count($r_pager);
        $pager = new ShowPager($total, $pagesize, $page);

        $sql = "select * from lkt_extension where store_id = '$store_id' and store_type = '$store_type' order by add_date limit $start,$pagesize ";
        $r = $db->select($sql);
