...
class userAction extends Apimiddle {
    ...
    public function index(){
        ...
        if(!$openid){
            echo json_encode(array('status'=>0,'err'=>'参数缺少！'));
            exit();
        }
        ...
        // $logo = $img.$logo;
        $logo = 'https://open.laiketui.com/logo.png';
        
        // 获取文章信息
        $sql_2 = "select Article_id,Article_prompt,Article_title from lkt_article where store_id = '$store_id' and `store_type` = '$store_type'";
        $r_2 = $db->select($sql_2);
        if($r_2){
            $article = $r_2;
        }else{
        ...
        // 查询会员信息
        $sql = "select * from lkt_user where wx_id = '$openid' and store_id = '$store_id'";
        $r = $db -> select($sql);
        ...
        // 查询会员信息
        $sqlu = "select u.user_name from lkt_user_distribution as d LEFT JOIN lkt_user as u  ON d.pid = u.user_id where d.user_id = '$user_id' and  u.store_id = '$store_id' ";
        $ru = $db -> select($sqlu);
        if($ru){
        ...
        $us_data = $user_page_data ? $user_page_data[0]->value:json_encode($Navbar->get_user_pages());
        $data = json_decode($us_data);
        ...
        foreach ($order as $key => $value) {
            //按照规则截取
            $status =  substr($key,strripos($key,"_")+1);
            $sql_order = "select num from lkt_order where status = '$status' and  user_id = '$user_id' and store_id = '$store_id'" ;
            $order_num = $db -> selectrow($sql_order);
            //获取url
            if(array_key_exists($value->text, $links)){
                $value->link = $links[$value->text];
            }
            //小红点数字
            $value->num = $order_num;
            $res_order[$status] =  $value; 
        }

        $data->orders = $res_order; 
        

        $support = '来客电商提供技术支持';

        echo json_encode(array('status'=>1,'support'=>$support,'tjr'=>$tjr,'user'=>$user,'company'=>$company,'logo'=>$logo,'article'=>$article,'order'=>$data));
        exit();
        return;
    }
    // 同步资料
    public function material(){
        ...
        $sql = "update lkt_user set user_name='$nickName',wx_name='$nickName',sex='$gender',headimgurl='$avatarUrl' where store_id = '$store_id' and wx_id = '$openid'";
        $r = $db->update($sql);
    ...
    public function verify_paw(){
        ...
        // 验证密码是否存在 或是否设置
        $sql = "select password from lkt_user where store_id = '$store_id' and wx_id = '$openid' $and";
        $r = $db -> select($sql);
        if($r){
    ...
    public function details(){
        ...
        $r_1 = $db->select($sql);
        $user['min_amount'] = $r_1 ? $r_1[0]->min_amount:1; // 最小提现金额
        $user['max_amount'] = $r_1 ? $r_1[0]->max_amount:1000; // 最大提现金额
        $user['unit'] = $r_1 ? $r_1[0]->unit:'元'; // 单位
        $user['multiple'] = $r_1 ? $r_1[0]->multiple:1; // 提现倍数
        ...
        if($r_2){
            ...
            $sql = "select * from lkt_user_bank_card where store_id = '$store_id' and user_id = '$user_id' and is_default = 1";
            $r_3 = $db->select($sql);
            if($r_3){
        ...
        // 根据推荐人等于会员编号,查询推荐人总数
        $sql = "select count(Referee) as a from lkt_user where store_id = '$store_id' and Referee = '$user_id'";
        $r_3 = $db -> select($sql);
        $user['invitation_num'] = $r_3[0]->a;
        // 根据微信id,查询分享列表里的礼券总和
        $sql = "select sum(coupon) as a from lkt_share where store_id = '$store_id' and wx_id = '$openid'";
        $r_4 = $db->select($sql);
        if($r_4[0]->a == ''){
            $user['coupon'] = 0;
        }else{
        ...
        $sql = "select money,add_date,type from lkt_record where store_id = '$store_id' and user_id = '$user_id' order by add_date desc";
        $r_5 = $db->select($sql);

        $list_1 = [];
        if($r_5){
            foreach ($r_5 as $k => $v) {
                if($v->type == 1 ||$v->type == 4 ||$v->type == 5 || $v->type == 6 ||$v->type == 11||$v->type == 12||$v->type == 13||$v->type == 14||$v->type == 23||$v->type == 24||$v->type == 25){
        ...
        $sql = "select money,add_date,type from lkt_record where (store_id = '$store_id' and user_id = '$user_id' and type = 21) or (store_id = '$store_id' and user_id = '$user_id' and type = 22) order by add_date desc";
        $r_6 = $db->select($sql);
        if($r_6){
            foreach ($r_6 as $k => $v) {
                $v->time = substr($v->add_date,0,strrpos($v->add_date,':'));
    ...
    public function withdrawals(){
        ...
        if(is_numeric($Bank_card_number) == false){
            ...
            if(strlen($Bank_card_number) != 19 && strlen($Bank_card_number) != 16){
                echo json_encode(array('code'=>114,'message'=>'卡号为16或19位数字!'));
                exit();
            }
        }
        // 根据卡号,查询银行名称
        ...
        // 根据wx_id查询会员id
        $sql = "select money,user_name,user_id from lkt_user where store_id = '$store_id' and wx_id = '$openid'";
        $r = $db->select($sql);
        ...
        if($count > 0){
            ...
            $sql = "update lkt_user set money = money - '$jine' where store_id = '$store_id' and wx_id = '$openid'";
            $res = $db->update($sql);
            // 在提现列表里添加一条数据
            $sql = "insert into lkt_withdraw (store_id,name,user_id,wx_id,mobile,Bank_id,money,s_charge,status,add_date) values ('$store_id','$user_name','$user_id','$openid','$mobile','$bank_id','$amoney','$cost',0,CURRENT_TIMESTAMP)";
            $res = $db->insert($sql);
            if($res == 1){
                $event = $user_id.'申请提现'.$jine.'元余额';
                $user_money = $r[0]->money;
                $sqll = "insert into lkt_record (store_id,user_id,money,oldmoney,event,type) values ('$store_id','$user_id','$jine','$user_money','$event',2)";
                $db->insert($sqll);
    ...
    public function share(){
        ...
        if($n == 0){
            // 根据新闻id,查询新闻信息
            $sql = "select * from lkt_news_list where store_id = '$store_id' and id = '$id'";
            $r = $db->select($sql);
            ...
            if(!empty($total_amount) && $total_num !=1 ){
                ...
                // 把剩余金额替换原数据库金额
                $sql = "update lkt_news_list set total_amount=$total_amount,total_num='$total_num'-1 where store_id = '$store_id' and id = '$id'";
                $r = $db->update($sql);

                $sex = $r[0]->sex; // 性别
                // 在分享列表添加一条数据
                $sql = "insert into lkt_share (store_id,user_id,wx_id,wx_name,sex,type,Article_id,coupon) values ('$store_id','$user_id','$openid','$wx_name','$sex','$n','$id','$money')";
                $r = $db->insert($sql);

                $sql = "update lkt_user set money = money+'$money' where store_id = '$store_id' and wx_id = '$openid'";
                $r = $db->update($sql);

                //添加日志

                $event = $user_id.'分享获得了'.$money.'元';
                $sqll = "insert into lkt_record (store_id,user_id,money,oldmoney,event,type) values ('$store_id','$user_id','$money','$ymoney','$event',24)";
                $rr = $db->insert($sqll);
            ...
            if(!empty($total_amount) && $total_num !=1 ){
                ...
                $total_amount=$total_amount-$money; // 剩余金额
                // 把剩余金额替换原数据库金额
                $sql = "update lkt_article set total_amount=$total_amount,total_num='$total_num'-1 where store_id = '$store_id' and Article_id = '$id'";
                $r = $db->update($sql);
                // 根据wxid,查询会员信息
                $sql = "select * from lkt_user where store_id = '$store_id' and wx_id = '$openid'";
                $r = $db->select($sql);
    ...
    public function SaveAddress(){
        ...
        if(preg_match("/^(13[0-9]|14[579]|15[0-3,5-9]|16[6]|17[0135678]|18[0-9]|19[89])\\d{8}$/", $mobile)){
            ...
            $r = $db->select($sql);
            if ($r) {
                $sql = "insert into lkt_user_address(store_id,name,tel,sheng,city,quyu,address,address_xq,uid,is_default) values('$store_id','$user_name','$mobile','$sheng','$shi','$xian','$address','$address_xq','$user_id',0)";
                $rr = $db->insert($sql);
            } else {
                $sql = "insert into lkt_user_address(store_id,name,tel,sheng,city,quyu,address,address_xq,uid,is_default) values('$store_id','$user_name','$mobile','$sheng','$shi','$xian','$address','$address_xq','$user_id',1)";
                $rr = $db->insert($sql);
            }
            if($rr >= 0){
    ...
    public function selectuser(){
        ...
        $sql001 = "select * from lkt_user where store_id = '$store_id' and wx_id = '$openid'";
        $r001 = $db->select($sql001);

        // 查询余额参数表
        $sql0001 = "select * from lkt_finance_config where store_id = '$store_id'";
        $r0001 = $db->select($sql0001);
        $transfer_multiple = $r0001[0]->transfer_multiple;
        if(!empty($r)){

            if($r001[0]->user_id == $user_id){
                echo json_encode(array('status'=>0,'err'=>'不能选择自己！'));
                exit();
            }
    ...
    public function transfer(){
        ...
        if($money <= 0 || $money == ''){
            ...
            $sql002 = "select money from lkt_user where store_id = '$store_id' and user_id = '$user_id'";
            ...
            if($r001 && $r002){
                ...
                $sql01 = "update lkt_user set money = money - '$money'  where store_id = '$store_id' and wx_id = '$openid' and money > 0 ";
                $r01 = $db->update($sql01);//本人
                $sql02 = "update lkt_user set money = money + '$money'  where store_id = '$store_id' and user_id = '$user_id' ";
                $r02 = $db->update($sql02);//好友

                $sql0001 = "insert into lkt_record (store_id,user_id,money,oldmoney,add_date,event,type) values ('$store_id','$user_id001','$money','$money001','$date_time','转账给好友','12')"; //本人
                $r0001 = $db->insert($sql0001);
                $sql0002 = "insert into lkt_record (store_id,user_id,money,oldmoney,add_date,event,type) values ('$store_id','$user_id','$money','$money002','$date_time','好友转账','13')";//好友
    ...
    public function perfect_index()
    {
        ...
        $sql002 = "select real_name as name,mobile,sex,province,city,county,wechat_id,birthday from lkt_user where store_id = '$store_id' and user_id = '$user_id'";
        $r002 = $db->select($sql002);//好友
        // var_dump($r002);
        if($r002){
    ...
    public function perfect()
    {
        ...
        $sql02 = "update lkt_user set real_name = '$name',mobile='$mobile',sex='$sex',province='$province',city='$city',county='$county',wechat_id='$wx_id',birthday='$date' where store_id = '$store_id' and user_id = '$user_id'";
