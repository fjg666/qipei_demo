...
class addFavoritesAction extends Apimiddle {
    /*
    时间2018年03月13日
    修改内容：修改首页商品及分类请求数据
    修改人：苏涛
...
//        }
    ...
    public function collection(){
        ...
        $sql ="select l.id,a.id as pid,a.product_title,a.imgurl as img,min(c.price) as price from lkt_user_collection as l left join lkt_product_list AS a on l.p_id = a.id RIGHT JOIN lkt_configure AS c ON a.id = c.pid where a.store_id = '$store_id' and l.user_id = '$user_id' and a.num >0 group by c.pid order by l.add_time desc";
        $r = $db->select($sql);
        $arr = [];
        if($r){
            foreach ($r as $k => $v) {
                ...
                $array['price']=$v->price;
                $array['imgurl']= ServerPath::getimgpath($v->img);
                $v = (object)$array;
                $arr[$k] = $v;
            }
    ...
    public function removeFavorites(){
        ...
        $id = $_POST['id']; // 收藏id
        // 根据收藏id,删除收藏表信息
        $sql = "delete from lkt_user_collection where store_id = '$store_id' and id = '$id'";
        $r = $db->delete($sql);
        if($r > 0){
    ...
    public function alldel()
    {
        ...
        $sql = "delete from lkt_user_collection where store_id = '$store_id' and user_id = '$userid'";
        $r = $db->delete($sql);
        if ($r){
