...
class addressAction extends Apimiddle {
...
//        }
        ...
        if($r){
            ...
            if (!empty($arr)) {
                ...
                foreach ($arrid as $key => $value) {
                    if ($value != '') {
                        $sql = "delete from lkt_user_address where store_id = '$store_id' and uid = '$user_id' and id = '$value'";
                        $r = $db->delete($sql);
                    }
    ...
    public function del_adds(){
        ...
        // 根据用户id,查询地址表
        $sql = "delete from lkt_user_address where store_id = '$store_id' and uid = '$user_id' and id = '$id_arr'";
        $r = $db->delete($sql);
        if($r > 0){
    ...
    public function up_adds(){
        ...
        $sql = "select * from lkt_user_address where store_id = '$store_id' and id = '$id_arr'"; //查询修改前的详细地址
        $r = $db->select($sql);
        if($r){
            $uid = $r[0]->uid;//用户ID
        ...
        if(preg_match("/^1[34578]\d{9}$/", $mobile)){
            $sql04 = "update lkt_user_address set name = '$user_name',tel= '$mobile',sheng='$sheng',city ='$shi',quyu ='$xian',address ='$address',address_xq ='$address_xq',code ='$code',uid ='$uid',is_default='$is_default' where store_id = '$store_id' and id = '$id_arr'";
            $r04 = $db->update($sql04);
            if($r04 ==1){
