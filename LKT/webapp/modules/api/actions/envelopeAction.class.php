...
class envelopeAction extends Apimiddle {
...
//        }
    ...
    public function index(){
        ...
        if($r){
            ...
            $r['0']->Article_imgurl = ServerPath::getimgpath($r['0']->Article_imgurl); // 图片
            $content = $r[0]->content;
    ...
    public function share(){
        ...
        /* ----- 分享成功 -----*/
        // 根据文章id,修改文章分享次数
        $sql = "update lkt_article set share_num = share_num+1 where store_id = '$store_id' and Article_id = '$id'";
        $r = $db->update($sql);
        // 根据wx_id,修改会员分享次数
        $sql = "update lkt_user set share_num = share_num+1 where store_id = '$store_id' and wx_id = '$openid'";
        $r = $db->update($sql);
        // 根据wx_id查询会员id
        ...
        // 在操作列表添加一条分享信息
        $sql = "insert into lkt_record (store_id,user_id,event,type) values ('$store_id','$user_id','$event',3)";
        $r = $db->insert($sql);
        ...
        if($total_amount != 0 && $total_num !=0){
            ...
            // 微信id和当天时间内,查询分享列表
            $sql = "select * from lkt_share where store_id = '$store_id' and wx_id = '$openid' and Article_id = '$id' and '$start_time'<=share_add and share_add<'$end_time'";
            $r = $db->select($sql);
