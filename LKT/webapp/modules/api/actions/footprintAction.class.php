...
class footprintAction extends Apimiddle {
...
//        }
    ...
    public function index(){
        ...
        if($r_1){
            ...
            foreach ($r_1 as $k_1 => $v_1) {
                $p_id = $v_1->p_id;
                $sql ="select a.id,a.product_title,a.imgurl,a.volume,min(c.price) as price,c.yprice,c.img,c.id AS sizeid from lkt_product_list AS a RIGHT JOIN lkt_configure AS c ON a.id = c.pid where a.store_id = '$store_id' and a.id ='$p_id' group by c.pid ";
                $rr_1 = $db->select($sql);
                if($rr_1){
                    foreach ($rr_1 as $key_1 => $value_1) {
                        $value_1->imgurl = ServerPath::getimgpath($value_1->imgurl); // 拼图片路径
                        $res_1[$k_1] = $value_1;
                    }
                }
            }
        ...
        if($r_2){
            ...
            foreach ($r_2 as $k_2 => $v_2) {
                ...
                $sql ="select a.id,a.product_title,a.volume,min(c.price) as price,c.yprice,c.img,c.id AS sizeid from lkt_product_list AS a RIGHT JOIN lkt_configure AS c ON a.id = c.pid where a.store_id = '$store_id' and a.id ='$p_id' and a.num >0 group by c.pid ";
    ...
    public function alldel()
    {
        ...
        $sql_user = 'select user_id from lkt_user where store_id = \''.$store_id.'\' and wx_id=\''.$openid.'\'';
        ...
        $sql = "delete from lkt_user_footprint where store_id = '$store_id' and user_id = '$userid'";
        $r = $db->delete($sql);
        if ($r){
