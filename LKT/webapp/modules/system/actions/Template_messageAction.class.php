...
class Template_messageAction extends Action {
    ...
    public function execute(){
        ...
        $log = new LaiKeLogUtils("app/system.log");

        $notice= $request->getParameter('notice'); 
        $sql = "select * from lkt_notice where store_id = '$store_id'";
        $num = $db->selectrow($sql);
        if($num < 1){
            $notice['store_id'] = $store_id;
            $r = $db->insert_array($notice,'lkt_notice');
        }else{
            $r = $db->modify($notice,'lkt_notice'," store_id = '$store_id' ");
        }
        if($r == -1) {
            $db->admin_record($store_id,$admin_name,'修改修改消息管理失败！',2);
            $log -> customerLog(__LINE__.":修改修改消息管理失败：".json_encode($notice)." \r\n");
            echo "<script type='text/javascript'>" .
                "alert('未知原因，修改失败！');" .
                "location.href='index.php?module=system&action=Template_message';</script>";
            return $this->getDefaultView();
        } else {
            $db->admin_record($store_id,$admin_name,'修改修改消息管理成功！',2);
            $log -> customerLog(__LINE__.":修改修改消息管理成功！ \r\n");
