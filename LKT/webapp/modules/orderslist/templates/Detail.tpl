    ...
    <form name="form1" id="form1" class="form form-horizontal" method="post" enctype="multipart/form-data">
        ...
        <div style="margin-bottom: 20px;background-color: #fff;">
            ...
            <ul class="ul1" style="height: 180px;">
                ...
                <li style="width: 25%;">订单状态：<span class="grText">{$data.r_status}</span></li>
                    ...
                    {else}
                        ...
                        {else}
                            <div style="display: flex;flex-direction:column;justify-content:center;line-height: 20px!important;">
                                {foreach from=$data.courier_num item=item2}
                                    <a class="send-btn1 goods-name" href="javascript:"
                                       style="display: block;width:200px;"
                                       onclick="send_btn1(this,'{$data.sNo}','{$item2.num}')">
                                        <span class="grText changeNum">{$item2.num} ({$item2.kuaidi_name})</span>
                                    </a>
                                {/foreach}
                            </div>
                        {/if}
            ...
            <ul class="ul2" style="height: 250px;">
                ...
                <li style="width: 100%;">备注信息：
                    {if $update_s}
                        ...
                        {if $data.gstatus < 2}
                            <input type="text" style="min-width: 250px;" name="updata[remarks]" value="{$data.remarks}">
                        {else}
                            <span class="grText">{$data.remarks}</span>
                        {/if}
                    {else}
                        <span class="grText">{$data.remarks}</span>
                    {/if}
                ...
                {foreach from=$detail item=item name=f1}
                    <tr>
                        <td style="text-align:left;" id="p_name" style="width: 30%">
                            <img class='pimg' src="{$item->pic}" style="margin-right: 20px;margin-left: 15px;" width="50" height="50"/>{$item->p_name}
                        </td>
                        <td class="center"><span class="grText">{$item->p_id}</span></td>
                        <td class="center" rowspan="{$item->index}"><span class="grText">{$item->num}</span></td>
                        <td class="center" rowspan="{$item->index}"><span class="grText" style="display: flex;align-items: center;">
                                 {if $data.otype=='KJ' || $data.otype=='JP'}
                                     {$item->spz_price}
                                 {elseif $data.otype=='integral'}
                                     ￥{$item->p_money}+<img src="images/icon/integral_hei.png" alt="" style="width: 15px;height: 15px;">{$item->allow}
                                 {else}
                                     ￥{$item->p_price}
                                 {/if}
                        ...
                        <td class="center grade_rate" rowspan="{$item->index}">{$item->grade_rate}</td>
                        <td class="center" rowspan="{$item->index}"><span class="grText">{$item->total_num}</span></td>
                        ...
                        <td class="center">
                            <span class="grText" style="display: flex;align-items: center;">
                                ￥
                                {if $data.otype=='KJ' || $data.otype=='JP'}
                                   {$item->z_price*$item->grade_rate2+$item->freight}
                                 {elseif $data.otype=='integral'}
                                    {$item->num*$item->p_price+$item->freight}+<img src="images/icon/integral_hei.png" alt="" style="width: 15px;height: 15px;">{$item->allow}
                                {else}
                                    {$item->num*$item->p_price*$item->grade_rate2+$item->freight}
                                {/if}
                            </span>
                        </td>
                        <td class="center">
                            <div style="display: flex;height: 100%;">
                                {if $button[0] == 1}
                                    {if $data.otype=='pt'&&$data.gstatus == 10}
                                        <a style="text-decoration:none;width: 44%;" class="tml-5 tml_flex"
                                           onclick="confirm('确认要退款吗？','{$item->id}')">
                ...
                {if $zp_res}
                    <tr>
                        <td style="text-align:left;display: flex;align-items: center;" id="p_name">
                            <img class='pimg' src="{$zp_res->img}" style="margin-right: 20px;margin-left: 15px;"
                                 width="50"
                                 height="50"/>
                            <span style="overflow: hidden;text-overflow:ellipsis;white-space: nowrap;width: 250px;font-size: 14px;">{$zp_res->product_title}</span>
                        </td>
                        <td class="center"><span class="grText">{$zp_res->pid}</span></td>
                        <td class="center"><span class="grText">1</span></td>
                        <td class="center"><span class="grText">￥{$zp_res->price}</span>
                        <td class="center"><span class="grText">-</span>
                        </td>
                        <td class="center"><span class="grText">{$zp_res->total_num}</span></td>
                        <td class="center"><span class="grText">{$zp_res->id}</span></td>
                        <td class="center">
                        ...
                        <tr>
                            <td colspan="8" style="text-align: right;">
                                <p><span style="color:#000000;">优惠合计：</span><span
                                            style="color:#FF0000;">￥{$yh_money}</span></p>
                            ...
                            <td>
                                <p style="text-align: center;"><span style="color:#000000;">合计支付：</span>
                                    ...
                                    {if $data.otype=='integral'}
                                        <span style="color:#FF0000;">+<img src="images/icon/integral_hei.png" alt="" style="width: 15px;height: 15px;">{$item->allow}</span>
                                    {/if}
                                </p>
