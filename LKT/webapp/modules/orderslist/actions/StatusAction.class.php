...
class StatusAction extends Action
{
    ...
    public function getDefaultView()
    {
        ...
        $sqld = 'update lkt_order_detail set status=' . $trade . 'where store_id = "' . $store_id . '" and sNo="' . $sNo . '"';
    ...
    public function execute()
    {
        ...
        $db->begin();
        $code = true;
        $sqld = "update lkt_order set status=11,refundsNo='$refund' where store_id = '$store_id' and sNo='$sNo'";
        $resd = $db->update($sqld);
        if ($resd < 0) {
            $lktlog->customerLog(__METHOD__ . ":" . __LINE__ . "修改订单状态失败！sql:" . $sqld);

            $code = false;
        }
        if ($paytype == 'wallet_Pay') {
            $oldmoney = $db->select("select money from lkt_user where store_id = '$store_id' and user_id='$uid'");
            $oldmoney = $oldmoney[0]->money;

            $sql = "update lkt_user set money=money+$price where store_id = '$store_id' and user_id='$uid'";
            $res = $db->update($sql);
            if ($res < 0) {
                $lktlog->customerLog(__METHOD__ . ":" . __LINE__ . "修改用户金额失败！sql:" . $sql);

                $code = false;
            }
            $date = date('Y-m-d H:i:s');
            $recordsql = "insert into lkt_record(store_id,user_id,money,oldmoney,add_date,event,type) values('$store_id','$uid',$price,$oldmoney,'$date','" . $uid . "拼团失败退款',5)";
            $res_recordsql = $db->insert($recordsql);
            if ($res_recordsql < 0) {
                $lktlog->customerLog(__METHOD__ . ":" . __LINE__ . "添加记录失败！sql:" . $recordsql);
            }
        } else if ($paytype == 'wxPay') {
            $price1 = $price * 100;
            $res = $this->wxrefundapi($store_id, $trade_no, $refund, $price1);
        ...
        if ($code) {
            ...
            $usermsg = $db->select("select wx_id,user_name from lkt_user where store_id = '$store_id' and user_id='$uid'");
            if (!empty($usermsg)) $usermsg = $usermsg[0];
            $openid = $usermsg->wx_id;
            $fromidsql = "select fromid from lkt_user_fromid where store_id = '$store_id' and open_id='$openid' and id=(select max(id) from lkt_user_fromid where open_id='$openid')";
    ...
    */
    ...
    private function wxrefundapi($store_id, $ordersNo, $refund, $price)
    {
