...
class Model
{
	 ...
	 * @param $v
	 */
	public function  setCustomerAttr($k,$v){
		$this -> customerAttrs[$k] = $v;
		return $this ;
	}
	...
	public function  getAllAttrs(){
		return (object)array_merge($this -> customerAttrs,$this -> attributes);
	}
	 ...
	 * @param $v
	 */
	public function  getAttrsArrays(){
		return array_merge($this -> customerAttrs,$this -> attributes);
	}
	...
	public function assign_attribute($name, $value)
	{
		...
		if ($value instanceof \DateTime){
			$val = new DateTime($value->format('Y-m-d H:i:s'));
	...
	{
		return self::saveOrUpdate($sql,$values);
	}
	 ...
	 * @throws ActiveRecordException
	 */
	public static function add($sql, $values=array())
	{
		self::saveOrUpdate($sql,$values);
	 ...
	 * @throws ActiveRecordException
	 */
	public static function updateData($sql, $values=array())
	{
		return self::saveOrUpdate($sql,$values);
	}
	 ...
	 * @throws ActiveRecordException
	 */
	public static function saveOrUpdate($sql, $values=array())
	{
		try{
			static::table()->last_sql = $sql;
			$conn = static::connection();
			$ret = $conn->query($sql,$values);
			return $ret->rowCount();
		}catch (ActiveRecordException $e){
			throw new ActiveRecordException("Cannot updateOrSave, " .$sql);
		}
	}
...
//		var_dump($list);die;
	...
	{
		return static::find_by_sql($sql,$values);
	}
	 ...
	 * @return null
	 */
	private static function selectArray($sql, $values=null)
	{
		$res = static::table()->find_by_sql($sql, $values, true);
		if(empty($res)){
			return null;
		}
		$ret = array();
		foreach ($res as $k=>$v){
			$ret[$k] = $res[$k]->getAttrsArrays();
		}
		return $ret;
	}
	 ...
	 */
	public static function get($sql, $values=null){
		return self::selectArray($sql,$values);
	}
